...
const assertValidAccountKeyValue = (field, value, providerId) => {
    if (!value.trim() || invalidAccountKeyValues.has(value)) {
        throw new Error(`Account ${field} for providerId ${providerId} is not a valid Better Auth account identity`);
    }
};
const migrationCompleteCursorPrefix = "convex-better-auth:migration-complete:";
const migrationCompleteCursorFor = (operation) => `${migrationCompleteCursorPrefix}${operation}`;
const isMigrationCompleteCursor = (cursor, operation) => {
...
const assertValidProviderIssuers = (providerIssuers) => {
    for (const [providerId, issuer] of Object.entries(providerIssuers)) {
        ...
        if (invalidAccountKeyValues.has(issuer)) {
            throw new Error(`Trusted issuer mapping for providerId ${providerId} is not a valid Better Auth account identity`);
        }
        if (issuer !== issuer.trim()) {
            throw new Error(`Trusted issuer mapping for providerId ${providerId} contains leading or trailing whitespace`);
        }
...
const targetAccountIdentity = (account, providerIssuers) => {
    const accountId = account.providerId === "credential" ? account.userId : account.accountId;
    const fixedIssuer = account.providerId === "credential"
        ? "local:credential"
    ...
    if (existingIssuer !== undefined &&
        existingIssuer !== existingIssuer.trim()) {
        throw new Error(`Existing issuer for providerId ${account.providerId} contains leading or trailing whitespace`);
    }
    ...
    if (!fixedIssuer &&
        (issuer === "local:credential" || issuer === "local:siwe")) {
        throw new Error(`Issuer ${issuer} is reserved for a local authentication method`);
    }
    if (existingIssuer && existingIssuer !== issuer) {
        throw new Error(`Existing issuer ${existingIssuer} does not match trusted issuer ${issuer} for providerId ${account.providerId}`);
    }
...
const assertNoAccountIdentityCollision = async (ctx, account, providerIssuers) => {
    ...
    if (account.providerId === "credential") {
        const credentialCandidates = await ctx.db
            .query("account")
            .withIndex("providerId_userId", (index) => index.eq("providerId", "credential").eq("userId", account.userId))
            .collect();
        const candidateIds = new Set(candidates.map((candidate) => candidate._id));
        candidates.push(...credentialCandidates.filter((candidate) => !candidateIds.has(candidate._id)));
    }
    for (const candidate of candidates) {
        ...
        if (candidateTarget.issuer === target.issuer &&
            candidateTarget.accountId === target.accountId) {
            throw new Error(`Account identity collision for issuer ${target.issuer} and accountId ${target.accountId}`);
        }
    }
    return target;
};
...
export const clearLegacyOAuthProviderRecords = mutation({
    args: {
        table: v.union(v.literal("oauthApplication"), v.literal("oauthAccessToken"), v.literal("oauthConsent")),
        paginationOpts: paginationOptsValidator,
    },
    ...
    handler: async (ctx, args) => {
        ...
        const result = args.table === "oauthApplication"
...
export const validateAccountIssuerBackfill = query({
    ...
    handler: async (ctx, args) => {
        ...
        for (const account of result.page) {
            ...
            if (account.issuer === target.issuer &&
