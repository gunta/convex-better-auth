    ...
    do {
        ...
        const result = await next({
            paginationOpts: {
                ...
                numItems: Math.min(numItems ?? 200, limit !== undefined ? limit - state.docs.length : Infinity, 200),
                cursor: state.cursor,
            },
        });
        onResult(result);
        ...
        if (!state.isDone && !advanced && !produced) {
            throw new Error("handlePagination made no forward progress; aborting to avoid an infinite pagination loop");
        }
    } while (!state.isDone);
    return state;
};
const parseWhere = (where) => {
    ...
    for (const w of whereArray) {
        if (w.mode === "insensitive") {
            throw new Error(`Case-insensitive queries (mode: "insensitive") are not supported by the Convex adapter. Store values in a normalized form (e.g. lowercase on write) and query against the normalized value. Field: ${w.field}`);
        }
    }
        ...
        adapter: ({ options }) => {
            ...
            if (options.advanced?.database?.joins) {
                ...
                if (!didWarnJoinsUnsupported) {
                    ...
                    console.warn("[convex-better-auth] Better Auth advanced.database.joins is not supported by the Convex adapter yet. Forcing advanced.database.joins = false.");
                }
            }
