const resolveCdTarget = (relativeDir, cwdBasename) => {
    if (relativeDir === "") {
        return null;
    }
    if (relativeDir !== null) {
...
const mergedIndexFields = (tables) => Object.fromEntries(Object.entries(tables).map(([key, table]) => {
    const schemaIndexes = table.indexes?.map((index) => index.fields.map((field) => table.fields[field]?.fieldName ?? field)) ?? [];
    ...
    const indexes = schemaIndexes.concat(manualIndexes, specialFieldIndexes.map((field) => [field]));
    const deduplicated = indexes.filter((index, indexPosition) => {
        return (indexes.findIndex((candidate) => {
            return (candidate.length === index.length &&
...
export const createSchema = async ({ file, tables, }) => {
    ...
    const relativeDir = projectRoot !== null
