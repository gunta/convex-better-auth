...
const getUniqueConstraints = (betterAuthSchema, model) => {
    ...
    const tableConstraints = table.indexes
        ?.filter((index) => index.unique)
        .map((index) => index.fields.map((field) => table.fields[field]?.fieldName ?? field)) ?? [];
    return [...singleFieldConstraints, ...tableConstraints];
};
const isUniqueField = (betterAuthSchema, model, field) => getUniqueConstraints(betterAuthSchema, model).some((constraint) => constraint.length === 1 && constraint[0] === field);
export const hasUniqueFields = (betterAuthSchema, model, input) => {
    const inputFields = new Set(Object.keys(input));
    return getUniqueConstraints(betterAuthSchema, model).some((constraint) => constraint.every((field) => inputFields.has(field)));
};
export const touchesUniqueFields = (betterAuthSchema, model, input) => {
    const inputFields = new Set(Object.keys(input));
    return getUniqueConstraints(betterAuthSchema, model).some((constraint) => constraint.some((field) => inputFields.has(field)));
};
const findIndex = (schema, args) => {
    ...
    const index = !indexFields.length
        ...
        : (indexes.find(({ fields }) => matchesIndex(fields, true)) ??
            indexes.find(({ fields }) => matchesIndex(fields, false)));
    function matchesIndex(fields, requireExactEqOrder) {
        if (eqValues.size !== eqFields.length ||
        ...
        const suffixFieldsMatch = suffixFields.every((field, index) => field === fields[eqFields.length + index]);
        // If sorting by createdAt, no intermediate fields can be on the index
        // as they may override the createdAt sort order.
        const createdAtFieldMatch = boundField === "createdAt" || sortField === "createdAt"
    ...
    return {
        ...
        values: {
            eq: index.fields
                ...
                .map((field) => [field, eqValues.get(field)]),
...
export const checkUniqueFields = async (ctx, schema, betterAuthSchema, table, input, doc) => {
    ...
    for (const constraint of getUniqueConstraints(betterAuthSchema, table)) {
        if (!constraint.some((field) => field in input) ||
        ...
        if (indexConstraintFields.length !== constraint.length ||
            !indexConstraintFields.every((field) => constraint.includes(field))) {
            throw new Error(`Index ${index.indexDescriptor} does not match unique constraint ${table}.${constraint.join("+")}`);
        }
        const existingDocs = await ctx.db
            .query(table)
            .withIndex(index.indexDescriptor, (q) => indexConstraintFields.reduce((query, field) => query.eq(field, mergedDoc[field]), q))
            .take(2);
        if (existingDocs.some((existingDoc) => existingDoc._id !== doc?._id)) {
            const constraintName = constraint.length === 1
                ? constraint[0]
...
// is a select arg).
export const selectFields = (doc, select) => {
    if (!doc) {
        return null;
    }
    if (!select?.length) {
...
const generateQuery = (ctx, schema, args) => {
    ...
    const hasValues = (values?.eq?.length ?? 0) > 0 ||
        values?.lt !== undefined ||
        values?.lte !== undefined ||
        values?.gt !== undefined ||
        values?.gte !== undefined;
    const indexedQuery = usableIndex
        ? query.withIndex(usableIndex.indexDescriptor, hasValues
            ? (q) => {
...
export const paginate = async (ctx, schema, betterAuthSchema, args) => {
    ...
    if (args.where?.some((w) => w.mode === "insensitive")) {
        throw new Error(`Case-insensitive queries (mode: "insensitive") are not supported by the Convex adapter. Store values in a normalized form (e.g. lowercase on write) and query against the normalized value.`);
    }
    ...
    if (uniqueWhere) {
        ...
        const doc = uniqueWhere.field === "_id"
            ...
            : await ctx.db
                .query(args.model)
                .withIndex(index?.indexDescriptor, (q) => q.eq(uniqueWhere.field, uniqueWhere.value))
                .unique();
    ...
    if (inWhere) {
        ...
        if (inWhere.field === "_id") {
            const docs = await asyncMap(inWhere.value, async (value) => {
                return ctx.db.get(args.model, value);
            });
