...
const withBetterAuthOptions = (modifyBetterAuthOptions, options, test) => async () => {
...
export const convexCustomTestSuite = createTestSuite("convex-custom", {}, ({ adapter, getBetterAuthOptions, modifyBetterAuthOptions }) => ({
    ...
    "should support select in findMany": async () => {
        ...
        expect(result[0].email).toEqual("foo@bar.com");
    },
    ...
    "should roll back single-page bulk updates that collide on a compound unique constraint": async () => {
        await adapter.create({
            model: "account",
            data: {
                issuer: "issuer-a",
    ...
    "should apply a guarded increment only once": withBetterAuthOptions(modifyBetterAuthOptions, { rateLimit: { enabled: true, storage: "database" } }, async () => {
    ...
    "should increment a nullable legacy counter from zero": withBetterAuthOptions(modifyBetterAuthOptions, { plugins: [twoFactor()] }, async () => {
...
//# sourceMappingURL=convex-custom.js.map