...
const pickTests = (tests, testNames) => {
    const picked = Object.fromEntries(Object.entries(tests).filter(([testName]) => testNames.includes(testName)));
    const missing = testNames.filter((name) => !(name in picked));
    if (missing.length > 0) {
        throw new Error(`Upstream test name drift: ${missing.length} test(s) not found: ${missing.join(", ")}`);
    }
    return picked;
};
...
export const coreAuthFlowTestSuite = createTestSuite("auth-flow", {
    ...
    if (!(ADDITIONAL_FIELDS_AUTH_FLOW_TEST in tests)) {
        throw new Error(`Upstream test name drift: auth-flow test not found: ${ADDITIONAL_FIELDS_AUTH_FLOW_TEST}`);
    }
