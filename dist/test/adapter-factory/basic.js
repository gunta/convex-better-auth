...
const toMillis = (value) => value instanceof Date ? value.getTime() : value;
export const getNormalTestSuiteTests = (helpers, debugTools) => {
    const tests = getUpstreamNormalTestSuiteTests(helpers, debugTools);
    const { adapter, generate, getBetterAuthOptions, insertRandom, modifyBetterAuthOptions, sortModels, transformGeneratedModel, } = helpers;
    return {
        ...tests,
        ...
        "findOne - should work with both one-to-one and one-to-many joins": async () => {
            ...
            if (result?.session?.length) {
                result.session = result.session.sort((a, b) => toMillis(a.createdAt) - toMillis(b.createdAt));
            }
            expect(result).toEqual({
                ...users[0],
        ...
        "findMany - should find many models with date fields": async () => {
            ...
            expect(sortModels(result)).toEqual(sortModels(users.filter((user) => toMillis(user.createdAt) < toMillis(youngestUser.createdAt))));
        },
        "findMany - should find many models with gt operator": async () => {
            ...
            const expectedResult = sortModels(users.filter((user) => toMillis(user.createdAt) > toMillis(oldestUser.createdAt)));
        ...
        "findMany - should find many models with gte operator": async () => {
            ...
            const expectedResult = users.filter((user) => toMillis(user.createdAt) >= toMillis(oldestUser.createdAt));
            expect(result.length).not.toBe(0);
